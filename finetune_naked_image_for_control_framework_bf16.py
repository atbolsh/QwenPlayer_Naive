...
Key changes from float32 version:
1. Loads finetune_control_step_001000.pt (float32) and converts to bf16
...
4. Uses a non-trainable Parameter for the 1/32 scaling (GPU-only, faster)
"""
...
# Default dtype - using bf16 for efficiency
...
LEDGER_PATH = os.path.join(os.path.dirname(__file__), "finetune_control_bf16_losses.csv")
...
class NakedImageAutoencoderWithRandomContext(nn.Module):
    """Standalone image autoencoder (bf16) with random context for decoder.
    ...
    Uses a non-trainable Parameter for the scaling factor to keep computation on GPU.
    """
    ...
    def __init__(self, embed_dim=1024, num_heads=8, dtype=DEFAULT_DTYPE, context_seq_len=32):
        ...
        # Non-trainable scaling factor (1/32 = 1/sqrt(1024)) as Parameter to keep on GPU
        self.register_buffer('context_scale', torch.tensor(1.0 / 32.0, dtype=dtype))
    ...
    def forward(self, img_batch):
        ...
        # Multiply by scale factor (1/32) to get ~1.0 magnitude per feature
...
def save_demo_image(model, step, device):
    ...
    with torch.no_grad():
        ...
        input_path = os.path.join(DEMO_DIR, f"finetune_control_bf16_step_{step:06d}_input.png")
        output_path = os.path.join(DEMO_DIR, f"finetune_control_bf16_step_{step:06d}_output.png")
...
# Initialize model (bf16)
print("Initializing NakedImageAutoencoderWithRandomContext (bf16)...")
...
# Load checkpoint (float32) and convert to bf16
if os.path.exists(INIT_CHECKPOINT):
    ...
    # Filter to only load img_enc and img_dec weights, convert to bf16
    filtered_state_dict = {}
    for key, value in state_dict.items():
        if key.startswith('img_enc.') or key.startswith('img_dec.'):
            filtered_state_dict[key] = value.to(DEFAULT_DTYPE)
    
    model.load_state_dict(filtered_state_dict, strict=False)
    print("Checkpoint loaded and converted to bf16 (img_enc and img_dec weights)!")
else:
...
for step in range(NUM_STEPS):
    ...
    if (step + 1) % SAVE_EVERY == 0:
        checkpoint_path = os.path.join(CHECKPOINT_DIR, f"finetune_control_bf16_step_{step+1:06d}.pt")
...
final_path = os.path.join(CHECKPOINT_DIR, "finetune_control_bf16_final.pt")
